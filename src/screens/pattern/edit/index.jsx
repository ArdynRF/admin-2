...
import { updatePattern } from "@/actions/patternActions";

const EditPattern = ({ pattern, searchParams }) => {
  const { errorMessage } = searchParams;

  return (
    <div>
      <h1 className="text-3xl font-semibold p-2">Edit Pattern</h1>

      <form
        className="grid gap-x-6 gap-y-10 mt-10 grid-cols-2 px-2"
        action={(formData) => updatePattern(formData, pattern.id)}
      >
        {errorMessage && (
